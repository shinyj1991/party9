<template>
  <article id="main" :style="{transform: `translateY(${headerPositionTop}px)`}" ref="header">
    <header class="header">
      ...
      <div class="menu">
        <button type="button" :class="{on: tabIndex === 0}" @click="tabAction(0)">파티원모집</button>
        <button type="button" :class="{on: tabIndex === 1}" @click="tabAction(1)">이벤트정보</button>
...
#main {overflow: hidden; position: relative; height: 100vh; padding: 160px 0 100px;
  .header {position: absolute; top: 0; right: 0; left: 0; z-index: 100; height: 160px; padding: 0 30px; border-bottom: 2px solid #b7b7b7; background: #ffffff;
    .head {display: flex; justify-content: space-between; align-items: center; height: 80px;
      .area {position: relative; height: 80px; padding: 0 30px 0 0; font-weight: 700; line-height: 80px; font-size: 32px;
    ...
    .menu {display: flex; position: relative;
      button {flex: 1 1 auto; height: 80px; font-size: 26px;
        &.on {font-weight: 700;}
      }
      ...
      ul {
        li {border-top: 2px solid #f1f4f7;
          ...
          a {display: block; position: relative; height: 250px; padding: 25px 0 25px 200px;
            .cate {position: absolute; top: 20px; left: 0; width: 160px; height: 200px; background: url(~static/images/common/default_thumbnail.png);}
